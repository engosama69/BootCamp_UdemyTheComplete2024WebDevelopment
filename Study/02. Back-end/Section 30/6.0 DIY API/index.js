...
app.get("/jokes/:id", (req, res) => {
  ...
  const foundJoke = jokes.find((joke) => joke.id === id);    // joke는 배열 내 현재 검사 중인 각 요소를 나타낸다.
...
app.get("/filter", (req, res) => {
  const type = req.query.type;   // type은 키(Key)의 값(Value)
  const filteredActivities = jokes.filter((joke) => joke.jokeType === type);   // 타입이 맞는 것을 찾아서 배열에 넣는다.
...
app.post("/jokes", (req, res) => {
  ...
  jokes.push(newJoke);
  console.log(jokes.slice(-1));     // 마지막에 있는(방금 푸시한) 항목 출력
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const existingJoke = jokes.find((joke) => joke.id === id);    // 존재하는 항목을 먼저 찾는다.
  const replacementJoke = {
    id: id,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    jokes.splice(searchIndex, 1);    // 배열에서 searchIndex에 있는 요소를 1개 삭제한다.
